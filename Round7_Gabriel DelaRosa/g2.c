...
void set_st(float *p_st, uint64 *buf);
...
uint64 y1;
	...
	x0 = g2_origin_x-10;
	y0 = g2_origin_y-10;
	x1 = g2_fontbuf_w + g2_origin_x;
	y1 = g2_fontbuf_h + g2_origin_y;
...
//---------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////////////////////
void g2_begin_trifan(int16 numpts, int32 w, int32 h, 
					 uint32 *data, uint32 *mip1, uint32 *mip2)
{
	uint32 mip_addr1, mip_addr2;
	int32 mip,alpha=1;

	gs_load_texture(0, 0, w, h, (uint32)data, g2_texbuf_addr, w);//g2_max_x+1);

	// load mip textures for levels 1 and 2
	mip = 0;
	if (mip1 && mip2)
	{
		mip_addr1 = g2_texbuf_addr + (w * h * g2_vbpp);
		gs_load_texture(0, 0, w>>1, h>>1, (uint32)mip1, mip_addr1, w>>1);

		mip_addr2 = mip_addr1 + ((w>>1) * (h>>1) * g2_vbpp);
		gs_load_texture(0, 0, w>>2, h>>2, (uint32)mip2, mip_addr2, w>>2);
		mip = 2;
		alpha=0;
	}

	BEGIN_GS_PACKET(gs_dma_buf);
	GIF_TAG_AD(gs_dma_buf, (numpts*5)+6, 1, 0, 0, 0);

	// Access the TEXFLUSH register with anything
	GIF_DATA_AD(gs_dma_buf, texflush, 0x42);

	// set number of mip levels ,filtering, and LOD
	GIF_DATA_AD(gs_dma_buf, tex1_1, GS_TEX1(0,mip,1,5,0,1,-270));//-470

	// set mip level addresses for 1 and 2
	GIF_DATA_AD(gs_dma_buf, miptbp1_1, GS_MIPTBP(mip_addr1/256,
												(w>>1)/64,
												mip_addr2/256,
												(w>>2)/64
												,0,0));
	...
	//	GS_CLAMP1(0,0,0,0,0,0));

	// define fog color
	GIF_DATA_AD(gs_dma_buf, fogcol, GS_FOGCOL(70,120,180));//80, 128, 200));
	...
	GIF_DATA_AD(gs_dma_buf, prim,
		GS_PRIM(PRIM_TRI_FAN, 1, 1, 1, alpha, 0, 0, 0, 0));//turn on gourad,texture,fog,alpha

}

void g2_add_pt_trifan(int32 x, int32 y, float z, float s, float t,float q)
{
	int32 fgz;
	x += g2_origin_x;
	y += g2_origin_y;
	...
	if (z > 20000.0f)
		...
		fgz = 255 - fptosi((z / 20000.0f) * 255.0f);

	GIF_DATA_AD(gs_dma_buf, st,    GS_ST(fptosi2(s),fptosi2(t) ));

	GIF_DATA_AD(gs_dma_buf, rgbaq,
		GS_RGBAQ(g2_col_r, g2_col_g, g2_col_b, 0x80, fptosi2(q)));

	GIF_DATA_AD(gs_dma_buf, fog, GS_FOG(255-fgz));

	GIF_DATA_AD(gs_dma_buf, alpha_1, GS_ALPHA(0,2,0,1,0));//Cs,0,As,Cd,0

	GIF_DATA_AD(gs_dma_buf, xyzf2, GS_XYZF2(x<<4, y<<4, fptosi(z)<<4, fgz));
}
